...
for jj=1:length(nuancenameFolds)
    ...
    aveintensity=zeros(numcell/2,numfea);
    pos=[];
    posroi=[];
    ...
    for kk=1:length(pic)
        ...
        im=im';
        for i=1:2:numcell %number of cells
            stri=num2str(i);
            name=[jjfolder,filesep,'RoiSet',filesep,dirOutput(i+2).name];
            [sROI] = ReadImageJROI(name);
            
            nameNu=[jjfolder,filesep,'RoiSet',filesep,dirOutput(i+3).name];
            [sROI_Nu] = ReadImageJROI(nameNu);           
